pipeline {
    ...
    stages {
        ...
        stage('users') {
            when {
                expression {
                sh(returnStatus: true, script: "git diff --name-only HEAD~1..HEAD| grep 'plays/users.*' > /dev/null") == 0
                }
            }
            steps {
        ...
        stage('iptables') {
            when {
                expression {
                sh(returnStatus: true, script: "git diff --name-only HEAD~1..HEAD| grep 'plays/iptables.*' > /dev/null") == 0
                }
            }
            steps {
